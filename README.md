...
> **Note:** C array parsing is currently provided only as a test/helper feature, and **only supports C files generated by `img2lv`**. Do not depend on it in production code. It may be changed or removed in a future release.
...
| `cf`          | `string`  | Color format (see table below), or `"AUTO"` / `"OPTIMIZED"`                |
...
When `isCArray` is `true`, the input is parsed from generated C source. This path is intended for testing and debugging only, and should not be relied on in production.
